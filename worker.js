...
// Default URL hosting the scored CSV output generated by the Python wrangler.
// The file is fetched at login so authenticated users can view real data
// instead of the hard-coded demo grants. The URL can be overridden via the
// `CSV_URL` environment variable configured in `wrangler.toml`.
const DEFAULT_CSV_URL =
  'https://raw.githubusercontent.com/asiakay/grant-manager-tool-demo/main/out/master.csv';

async function fetchGrantsFromCsv(csvUrl) {
  try {
...
export default {
  async fetch(request, env) {
    ...
    const csvUrl = env?.CSV_URL || DEFAULT_CSV_URL;
    ...
    if (url.pathname === '/dashboard') {
      ...
      const grants = await fetchGrantsFromCsv(csvUrl);
